...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def load_default_state_dict(self):
        # get params
        ckpt_path = "data/ckpt/music_speech_audioset_epoch_15_esc_89.98.pt"
        state_dict = torch.load(ckpt_path, weights_only=False)["state_dict"]

        k = "module.audio_branch."

        params = {
            key.removeprefix(k): state_dict[key]
            for key in filter(lambda x:x.startswith(k), state_dict.keys())
        }

        # load dict
        model_keys = set(self.state_dict().keys())
        filtered_dict = {k: v for k, v in params.items() if k in model_keys}

        self.load_state_dict(filtered_dict)


def create_htsat_model(audio_cfg=None, enable_fusion=False, fusion_type='None'):
